...
如果布尔表达式由布尔值和之乘积构成，那么称为**和之积式(POS)**.例如：$(\overline{A}+C)(B+C)$就是POS.

如果POS式中每一项都完全覆盖了这个表达式的域，那么称之为标准的和之积式(the Standard POS Form).这些因子也称为**最大项**.最大项的二进制表达式满足对应关系：$0$对应$A$,$1$对应$\overline{A}$.对于任意一个POS表达式，我们可以利用$(A+B)(A+C)=A+BC$来补足未出现的布尔值，例如：
